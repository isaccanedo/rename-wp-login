...
if ( defined( 'ABSPATH' ) && ! class_exists( 'Rename_WP_Login' ) ) {
	...
	class Rename_WP_Login {
		...
		public function admin_notices_incompatible() {
			echo '<div class="error"><p>' . sprintf( __( 'Atualize para a versão mais recente do WordPress para ativar %s.', 'rename-wp-login' ), '<strong>' . __( 'Rename wp-login.php', 'rename-wp-login' ) . '</strong>' ) . '</p></div>';
		}
		...
		public function wpmu_options() {
			...
			$out .= '<p>' . __( 'Esta opção permite que você defina um padrão padrão da rede, que pode ser substituído por sites individuais. Basta acessar as configurações de permalink do site para alterar a URL.', 'rename-wp-login' ) . '</p>';
		...
		public function admin_notices() {
			...
			if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
				echo '<div class="updated"><p>' . sprintf( __( 'Sua página de login agora está aqui: %s. Favoritar esta página!', 'rename-wp-login' ), '<strong><a href="' . $this->new_login_url() . '">' . $this->new_login_url() . '</a></strong>' ) . '</p></div>';
			}
		}
		...
		public function plugins_loaded() {
			...
			if (
				...
				wp_die( __( 'Este recurso não está ativado.', 'rename-wp-login' ) );
			}
		...
		public function wp_loaded() {
			...
			if ( is_admin() && ! is_user_logged_in() && ! defined( 'DOING_AJAX' ) ) {
				wp_die( __( '<strong>ERRO 404</strong> - P&aacute;gina n&atilde;o encontrada!', 'rename-wp-login' ) );
			}
